...
/* ----- Keep particles canvas synced with dynamic viewport ----- */
(function syncParticlesWithViewport(){
  const root = document.getElementById('particles-js');
  if (!root) return;

  const refresh = () => {
    // let CSS handle the container height — just ask particles.js to resize
    try { window.dispatchEvent(new Event('resize')); } catch (_) {}
  };

  // on load + viewport changes
